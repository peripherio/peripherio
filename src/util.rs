use error::{InvalidNumberError, InvalidJSONNumberError};
...
pub unsafe fn cast_to_ptr(type_str: &str, v: &Value) -> Result<*const u8, Error> {
    Ok(match v {
        ...
        Value::Number(n) => {
            match type_str {
                "number" => {
                    let via: f64 = n.as_f64().ok_or(InvalidJSONNumberError{ value: n.clone() })?;
                    mem::transmute::<&f64, *const u8>(&via) // f64
                },
                "integer" => {
                    let via: i64 = n.as_i64().ok_or(InvalidJSONNumberError{ value: n.clone() })?;
...
pub unsafe fn cast_from_ptr(type_str: &str, ptr: *const u8) -> Result<Value, Error> {
    Ok(match type_str {
        ...
        "integer" => {
            ...
            Value::Number(Number::from_f64(*f as f64).ok_or(InvalidNumberError{value: *f as f64})?)
        },
        "string" => {
